...
<body>
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="card">
							<div class="card-body">
								<form action="updatesubmittertaskact.action" method="post"
 ...
 <!-- 获取传入的select的选中项的项数， 将id为taskid的选中项改变为与其一致  -->  

                                           $("#taskid").val($("#taskid option:eq("+select.selectedIndex+")").val());                                                	}

                                                </script>
										<script>
 <!-- 设置id为task的值为request中的submittertask.task  -->  
...
$("#task").val("${submittertask.task}")</script>
 ...
 <!-- 获取传入的select的选中项的项数， 将id为taskidid的选中项改变为与其一致  -->  

                                           $("#taskidid").val($("#taskidid option:eq("+select.selectedIndex+")").val());                                                	}

                                                </script>
										<script>
 <!-- 设置id为taskid的值为request中的submittertask.taskid  -->  
...
$("#taskid").val("${submittertask.taskid}")</script>
									...
									<div class="form-group">
										...
										<select id='submitter' onchange="submitterchange(this)"
											...
											<!-- 使用c标签的if表达式判断submitterall是否为空 -->

											<c:if test="${ !empty submitterall}">

												<!-- 使用c标签的forEach表达式循环输出submitterall -->
 ...
 <!-- 获取传入的select的选中项的项数， 将id为submitterid的选中项改变为与其一致  -->  

                                           $("#submitterid").val($("#submitterid option:eq("+select.selectedIndex+")").val());                                                	}

                                                </script>
										<script>
 <!-- 设置id为submitter的值为request中的submittertask.submitter  -->  

$("#submitter").val("${submittertask.submitter}")</script>
 ...
 <!-- 获取传入的select的选中项的项数， 将id为submitteridid的选中项改变为与其一致  -->  

                                           $("#submitteridid").val($("#submitteridid option:eq("+select.selectedIndex+")").val());                                                	}

                                                </script>
										<script>
 <!-- 设置id为submitterid的值为request中的submittertask.submitterid  -->  

$("#submitterid").val("${submittertask.submitterid}")</script>
									...
									<div class="form-group">
										<label>任务文件</label>
										<!-- 定义任务文件input输入框，并从request中取值submittertask.taskfile赋值  -->
									...
									<div class="form-group">
										...
										<script>
 <!-- 将id为state的组件赋值为request中的submittertask.state字段 -->  
