											...
											<c:if test="${ !empty collectorall}">
												...
												<c:forEach items="${ collectorall}" var="item">
													...
													<tr>
														...
														<td>${item.tel }</td>
														<td>
															<!--  定义修改收集人按钮和删除收集人按钮，修改提交地址为updatecollector.action?id=${item.id}，删除提交地址为 deletecollector.action?id=${item.id}-->

															<a href="updatecollector.action?id=${item.id}">修改</a> <a
															href="deletecollector.action?id=${item.id}">删除</a>
